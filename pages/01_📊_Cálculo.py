...
#Equação MDE
st.write(f'<div style="text-align: left">{r"$$\text{MDE} = \\frac{{(z_{{\\alpha/2}} + z_\\beta) \\cdot SE}}{{\\text{{pooled\_prob}}}}$$"}</div>', unsafe_allow_html=True)
...
#Equação z de alpha
st.write(f'<div style="text-align: left">{r"$$\\text{z}_{\\alpha/2} = |{\\text{norm.ppf}}(\\alpha/2)|$$"}</div>', unsafe_allow_html=True)

#Equação z de beta
st.write(f'<div style="text-align: left">{r"$$\\text{z}_\\beta = |{\\text{norm.ppf}}(\\beta)|$$"}</div>', unsafe_allow_html=True)
